#!/bin/bash
# run_conformal_qwen.sh
# Run conformal search (minmax, softmax, margin) for Qwen model on all 9 datasets.

# Change directory to the repository root
cd "$(dirname "$0")/.."

GPU_ID=${1:-0}
RESULTS_DIR=${2:-"./conformal_results/qwen3-embedding-0.6b"}
...
echo "========================================================================"
echo "Running Conformal Hyperparameter Search for Qwen3-Embedding-0.6B"
echo "GPU ID:      $GPU_ID"
...
for DS in "${DATASETS[@]}"; do
    for METHOD in "${METHODS[@]}"; do
        ...
        python3 run_lbw_hyperparameter_search_conformal.py \
            ...
            --conformal_K 40 \
            --results_dir "$RESULTS_DIR" > "conformal_qwen3-embedding-0.6B_conformal_${METHOD}_${DS}.log" 2>&1
...
echo "All Qwen conformal jobs finished/launched."
