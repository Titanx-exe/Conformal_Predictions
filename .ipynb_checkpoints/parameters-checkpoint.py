...
class RankingParser(argparse.ArgumentParser):
    ...
    def add_general_args(self, args=None):
        ...
        # LBW (Look-Both-Ways) Configuration Arguments
        parser.add_argument(
            "--lbw_architecture",
            default="INPLACE",
            type=str,
            choices=["NONE", "INPLACE", "EXTEND", "EXTRA", "INTER"],
            help="LBW architecture type: NONE, INPLACE, EXTEND, EXTRA, INTER",
        )
        parser.add_argument(
            "--num_bidir_layers",
            default=0,
            type=int,
            help="Number of bidirectional attention layers (BIDIR)",
        )
        parser.add_argument(
            "--num_unsink_layers",
            default=0,
            type=int,
            help="Number of backward attention layers (BACK)",
        )
        parser.add_argument(
            "--mask_type",
            default="BACK",
            type=str,
            choices=["MASK0", "BACK"],
            help="Mask type for unsink layers: MASK0 or BACK",
        )
        parser.add_argument(
            "--model_id",
            default="Qwen/Qwen3-Embedding-0.6B",
            type=str,
            help="Model ID for Qwen3 models (e.g., Qwen/Qwen3-Embedding-0.6B, Qwen/Qwen3-Embedding-4B, Qwen/Qwen3-Embedding-8B)",
        )
        ...
        parser.add_argument(
            ...
            default="final",
            type=str,
            help="Which epoch checkpoint to load from the LoRA adapter (e.g., epoch_1, epoch_5, final). Default: final",
        )
