...
# Default model ID
DEFAULT_MODEL_ID = "./models_local/llama-3.2-1B"
...
def find_best_results(results: List[Dict]) -> Dict:
    ...
    return {"best_mrr": best_mrr, "best_ece": best_ece, "best_recall": best_recall}
...
def print_summary(all_results: List[Dict], timestamp: str, model_id: str = None):
    ...
    for i, r in enumerate(by_mrr[:3], 1):
        ...
        m = r["metrics"]
        print(
            f"{i}. {c['architecture']}, BIDIR={c['num_bidir_layers']}, BACK={c['num_unsink_layers']}, mask={c['mask_type']}"
        )
        ...
        json.dump(
            {
                ...
                "top3_by_mrr": [
                    {"config": r["config"], "metrics": r["metrics"]} for r in by_mrr[:3]
                ],
                "top3_by_recall": [
                ...
                "top3_by_ece": [
                    {"config": r["config"], "metrics": r["metrics"]} for r in by_ece[:3]
                ],
                "top3_overall": [
                    {"config": r["config"], "metrics": r["metrics"]}
                    for r in by_combined[:3]
                ],
            },
            f,
            indent=2,
...
def main():
    ...
    if args.test in ["all", "bidir"]:
        all_results.extend(
            run_bidir_tests(
                args.gpu_id, args.model_id, args.lora_adapter_path, args.lora_epoch
            )
        )
