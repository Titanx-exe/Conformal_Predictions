...
# Paths
# STAGE1_CHECKPOINT_PATH = "/data/upb/users/h/hpurohit/profiles/unix/cs/RR_Retrieval/Robust_Ranking/RobustRanking/Finetuning/finetuned_models/llama-3.2-1b-lora-dhnm_attn_mlp_blink_1m/epoch_3"
DATASET_PATH = "Datasets/blink_training_cleaned_1m.jsonl"
OUTPUT_DIR = "finetuned_models/qwen3-0.6b-lora-dhnm_attn_blink_1m"
LOSS_LOG_PATH = "Loss/qwen3-0.6b_dhnm_attn_mlp_blink_1m.jsonl"
    ...
    def __init__(self, path: str, max_samples: Optional[int] = None):
        ...
        with open(path, "r", encoding="utf-8") as f:
            for i, line in enumerate(f):
                ...
                self.pairs.append(
                    {"document": doc_text, "gold_entity": ent_text, "negatives": []}
                )
...
def collate_batch(
    batch: List[dict], tokenizer, max_length: int
...
#     dtype = torch.bfloat16 if torch.cuda.is_available() else torch.float32
#     base_model = AutoModel.from_pretrained(
#         MODEL_ID, dtype=dtype, attn_implementation="flash_attention_2"
#     )
#     base_model.config.use_cache = False

#     # === STAGE 2: Continue Training Stage 1 LoRA ===
#     if accelerator.is_main_process:
#         print(f"\n{'=' * 60}")
#         print(f"STAGE 2: Loading from {STAGE1_CHECKPOINT_PATH}")
#         print(f"{'=' * 60}\n")

#     # 1. Load the Stage 1 LoRA adapter directly onto the base model
#     # This loads the existing adapter weights (r=16, etc.) automatically from the checkpoint
#     model = PeftModel.from_pretrained(
#         base_model, STAGE1_CHECKPOINT_PATH, is_trainable=True
#     )

#     # 2. Enable gradients for the adapter layers so they can be updated
#     # Note: We do NOT merge_and_unload(), and we do NOT create a new LoraConfig.
#     # The model is already a valid PEFT model with trained weights.
#     model.enable_input_require_grads()

#     # 3. Enable gradient checkpointing (optional but recommended for memory)
...
#     if accelerator.is_main_process:
#         model.print_trainable_parameters()
#         print(f"\n✓ Stage 2 model ready: Resuming training on existing adapter.\n")
...
def mine_hard_negatives(
    ...
    per_gpu = (
        math.ceil(len(sampled_ents) / world_size)
        if is_distributed
        else len(sampled_ents)
    )
    ...
    with torch.no_grad():
        for i in tqdm(
            range(0, len(my_ents), encode_bs),
            desc=f"Rank {rank} encoding entities",
            disable=rank != 0,
        ):
    ...
    if local_embeds:
        ...
        local_tensor = torch.empty(
            0, model.config.hidden_size, device=DEVICE, dtype=torch.bfloat16
        )

    # Gather embeddings from all ranks
    if is_distributed:
        local_count = torch.tensor(
            [local_tensor.size(0)], device=DEVICE, dtype=torch.long
        )
        all_counts = [
        ...
        padded = torch.zeros(
            max_count, local_tensor.size(1), device=DEVICE, dtype=local_tensor.dtype
        )
    ...
    with torch.no_grad():
        for i in tqdm(
            range(my_doc_start, my_doc_end, encode_bs),
            desc=f"Rank {rank} querying",
            disable=rank != 0,
        ):
...
def train(model, tokenizer, dataset):
    ...
    if accelerator.is_main_process:
        ...
        print(
            f"Effective batch size: {BATCH_SIZE * accelerator.num_processes * GRADIENT_ACCUMULATION_STEPS}"
        )
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        for step, (doc_batch, ent_batch, labels) in enumerate(progress):
            ...
            with accelerator.accumulate(model):
                ...
                # Forward passes
                doc_out = model(**doc_batch)
                ent_out = model(**ent_batch)

                # Pool and normalize
                doc_emb = last_token_pool(
                    doc_out.last_hidden_state, doc_batch["attention_mask"]
                )
                ent_emb = last_token_pool(
                    ent_out.last_hidden_state, ent_batch["attention_mask"]
                )
            ...
            # Logging once per real optimizer update
            if did_sync:
                ...
                if global_step % LOG_EVERY_N_STEPS == 0 and accelerator.is_main_process:
                    avg_loss = epoch_loss / epoch_update_steps
