...
# def get_noncausal_attention_mask(self, attention_mask, input_shape, device=None, dtype=None):
#     """
#     Creates a bidirectional attention mask (all positions attend to all positions).
#     """
#     if self.config._attn_implementation == "flash_attention_2":
#         if attention_mask is not None and 0.0 in attention_mask:
#             return attention_mask
...
#     # Create a [batch, 1, seq, seq] mask - all zeros means all positions can attend
#     # Only mask out padding positions
#     if attention_mask.dim() == 2:
#         # attention_mask is [batch, seq] with 1 for valid, 0 for padding
#         # Expand to [batch, 1, 1, seq] then broadcast to [batch, 1, seq, seq]
#         extended_attention_mask = attention_mask[:, None, None, :].expand(-1, -1, seq_len, -1)
#     else:
#         extended_attention_mask = attention_mask[:, None, :, :]
