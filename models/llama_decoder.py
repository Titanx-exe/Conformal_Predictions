...
# C LlamaModel from Decoder folder
...
class LlamaDecoderRanker(torch.nn.Module):
    def __init__(self, device=None, params=None, 
        ...
        # Model configuration
        model_id = params.get('model_id', 'meta-llama/Llama-3.2-3B')  # or configurable via params
        ...
        # Load config and set LBW parameters
        self.config = AutoConfig.from_pretrained(model_id)
        self.config.pad_token_id = self.tokenizer.pad_token_id
        self.config._attn_implementation = "eager"  # or "flash_attention_2"
        
        # LBW Configuration
        # BIDIR: removes causal mask which means all tokens attend to all tokens
        # BACK: reversed causal mask which means each token attends to itself and subsequent tokens
        self.config.architecture = params.get('lbw_architecture', 'INPLACE')
        self.config.num_bidir_layers = params.get('num_bidir_layers', 10)    # BIDIR layers
        self.config.num_unsink_layers = params.get('num_unsink_layers', 0)   # BACK layers
        ...
        if lora_adapter_path is not None:
            ...
            self.model = PeftModel.from_pretrained(self.model, full_adapter_path)
        ...
        print(f"  BIDIR layers: {self.config.num_bidir_layers} (bidirectional attention)")
        print(f"  BACK layers: {self.config.num_unsink_layers} (backward attention, mask_type={self.config.mask_type})")
    ...
    def encode(self, batch):
        ...
        CRITICAL: Remove token_type_ids if present - LLaMA doesn't use them.
        """
    ...
    def forward(self, doc_input, context_len=None, target=None):
        ...
        embeddings = self.mean_pooling(output.last_hidden_state, doc_input['attention_mask'])
